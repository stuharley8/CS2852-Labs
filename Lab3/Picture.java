...
public class Picture {
    ...
    /**
     * Constructor that copies the dots from original into emptyList and uses it
     ...
     * @return the number of nanoseconds required for this algorithm to run
     * @param numDesired the desired number of dots
     * @throws IllegalArgumentException if numDesired < 3
     */
    public long removeDots(int numDesired) throws IllegalArgumentException {
    ...
    /**
     * Removes all but the numDesired most critical dots using iterators.
     * If the picture does not have more than numDesired dots, the methods returns
     * without changing the picture.
     * If numDesired < 3, and IllegalArgumentException is thrown.
     * @return the number of nanoseconds required for this algorithm to run
     ...
     * @throws IllegalArgumentException if numDesired < 3
     */
    public long removeDots2(int numDesired) throws IllegalArgumentException {
        long startTime = System.nanoTime();
        if(numDesired < 3) {
            throw new IllegalArgumentException("NumDesired less than 3");
        }
        while(numDesired < dots.size()) {
            ...
            Iterator<Dot> previousItr = dots.iterator();
            Iterator<Dot> nextItr = dots.iterator();
            Dot firstDot = nextItr.next();
            Dot secondDot = nextItr.next();
            itr.next();
            double lowCrit = itr.next().calculateCriticalValue(previousItr.next(), nextItr.next());
            int indexOfLowCrit = 1;
            double nextCritValue;
            for(int i = 2; i<dots.size()-1; i++) {
                nextCritValue = itr.next().calculateCriticalValue(previousItr.next(), nextItr.next());
                if(nextCritValue < lowCrit) {
                    lowCrit = nextCritValue;
                    indexOfLowCrit = i;
                }
            }
            Dot secondLastDot = previousItr.next();
            Dot lastDot = previousItr.next();
            nextCritValue = firstDot.calculateCriticalValue(lastDot, secondDot);
            if(nextCritValue < lowCrit) {
                indexOfLowCrit = 0;
            }
            nextCritValue = lastDot.calculateCriticalValue(secondLastDot, firstDot);
            if(nextCritValue < lowCrit) {
                indexOfLowCrit = dots.size()-1;
            }
            dots.remove(indexOfLowCrit);
